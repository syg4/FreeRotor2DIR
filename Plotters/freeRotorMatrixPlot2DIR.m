function [fig,had] = freeRotorMatrixPlot2DIR(dataMatrix,w1,w3,t2_array,dim,varargin)
...
for ii = 1:m
    for jj = 1:n
        ...
        [~, c] = contour(had(ii,jj),x,y,z,level_list);
        c.Fill = 'on';
        axis equal
        line([x(1) x(end)],[x(1) x(end)],'Color',[0 0 0]);
        colormap(map)
        caxis(ca);
        t2_lab = text(x(1)+0.02*range(x),y(end)-0.01*range(y),[num2str(t2_array(count)),' ps']); %t2 display
        t2_lab.FontSize = 11;
