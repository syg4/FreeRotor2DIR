function roVibHamiltonianMatrix = ...
    getRoVibHamiltonianMatrix_RotWave(v1, v2, j1, j2, m1, m2, gasConst)
...
roVibHamiltonianMatrix = overlap.*(v_e .* (v1 + 1/2) ...
    ...
    + B_e .* j1 .* (j1+1) ...
    - a_e .* (v1+1/2).*j1.*(j1+1) ...
    - D .* j1.^2 * (j1+1).^2 - v1.*2300);

roVibHamiltonianMatrix = (roVibHamiltonianMatrix - roVibHamiltonianMatrix(1,1)).*overlap;
